...
def generate_launch_description():
    ...
    urdf = os.path.join(
        get_package_share_directory("box_bot_description"), "robot/", "box_bot_v2.urdf"
    )
    urdf_file_name = "turtlebot3_" + "burger" + ".urdf"
    urdf = os.path.join(
        get_package_share_directory("turtlebot3_description"), "urdf", urdf_file_name
    )
    pkg_box_bot_description = get_package_share_directory("reinforcement_planning")
        ...
        spawn_robots_cmds.append(
            IncludeLaunchDescription(
                PythonLaunchDescriptionSource(
                    os.path.join(
                        pkg_box_bot_description, "launch", "spawn_robot.launch.py"
                    )
                ),
                launch_arguments={
                    "robot_urdf": urdf,
